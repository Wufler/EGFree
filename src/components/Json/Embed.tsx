      ...
      <div className="flex gap-4">
        ...
        <div className="grow">
          ...
          <div className="mb-2 text-sm">
            {messageContent.split(/(<@&\d+>)/).map((part) => {
              const roleMatch = part.match(/^<@&(\d+)>$/);
              if (roleMatch) {
                return (
                  <span
                    key={`role-${roleMatch[1]}`}
                          ...
                          <div className="mt-1 flex flex-wrap items-baseline gap-x-1 gap-y-0.5 text-sm">
                            ...
                            {settings.includeFooter && (
                              <PreviewTimestampChip
                                label={
                                  isCurrent ? "until" : "Free offer starts"
                                }
                                date={endDate}
                              />
                            )}
              ...
              >
                <div className="max-w-md bg-[#ffffff] dark:bg-[#2B2D31] border border-[#d4d7dc] dark:border-[#4e5058] rounded-r-sm p-3.5 pr-4">
                  ...
                  <div className="flex flex-col text-sm gap-0.5">
                    ...
                    {settings.includePrice &&
                      (!isCurrent ? (
                        game.price.totalPrice.originalPrice === 0 ? null : (
                          <span>
                            {game.price.totalPrice.discountPrice !==
                            game.price.totalPrice.originalPrice ? (
                              <span>
                      ...
                      ) : isPermanentlyFree(game) ? null : isDiscountedGame(
                          game,
                        ) ? (
                        <span>
                          <span className="line-through font-extralight">
          ...
          {parsedMobileGames
            ...
            .map((game) => {
              const endDate = game.promoEndDate
                ? new Date(game.promoEndDate)
                : null;
