...
import { loadBotState, getGuildSettings } from '../state'
...
export function getSettingsComponentsV2Payload(
	...
	if (currentCategory === 'main') {
		...
		const pcRoleLabel = s.mentionRoleId ? `<@&${s.mentionRoleId}>` : '*Not set*'
		...
		const mainRoleTitle = s.splitDesktopMobile ? 'PC Role' : 'Announcement Role'
